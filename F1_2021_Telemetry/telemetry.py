...
import struct
import F1_2021_Telemetry.defs as defs
import F1_2021_Telemetry.packets as packets
import F1_2021_Telemetry.utils as utils
...
def listener():
    while True:
        data, _ = sock.recvfrom(1464) # max possible size is 1464 bytes
        queue.append(data)

def stateUpdater():
    while True:
        packet = nextPacket()
        currentState.update(packet)

listenerThread = Thread(target=listener, daemon=True)
listenerThread.start()

updaterThread = Thread(target=stateUpdater, daemon=True)
updaterThread.start()
